(* ECDSA P-256 signature verification (FIPS 186-4, SEC 1) over Limbsx,
   ported from tinysvid sig/p256.ml and adapted for JOSE: an ES256
   signature is raw r || s, 64 bytes, not DER. Jacobian coordinates
   with a = -3; the point at infinity is Z = 0. NOT constant-time:
   verification handles only public inputs. Psychic signatures (r or s
   zero or >= the group order, CVE-2022-21449) are rejected before any
   point math. *)
...
let verify_limbs ~(x : int list) ~(y : int list) ~(digest : string)
    ~(r : int list) ~(s : int list) : bool =
...
(* ---------- JOSE-facing byte interface ---------- *)

let limbs_of_be_string (s : string) : int list =
  limbs_of_be_bytes (string_to_ints s)

(* A JWK coordinate is canonical only below the field prime. *)
let coord_in_field (c : string) : bool = cmp (limbs_of_be_string c) p_mod < 0

let on_curve_bytes ~(x : string) ~(y : string) : bool =
  on_curve (norm16 (limbs_of_be_string x)) (norm16 (limbs_of_be_string y))

(* Total split: the first n bytes and the rest. *)
let take_drop (n : int) (s : string) : string * string =
  let seq = String.to_seq s in
  (String.of_seq (Seq.take n seq), String.of_seq (Seq.drop n seq))

(* x, y: the 32-byte big-endian public coordinates (a JWK's decoded "x"
   and "y"). signature: the raw JOSE ES256 form, r || s, 64 bytes.
   input: the JWS signing input; it is hashed here. False on any
   invalid input, including a wrong-length signature. *)
let verify ~(x : string) ~(y : string) ~(input : string)
    ~(signature : string) : bool =
  match () with
  | () when not (Int.equal (String.length signature) 64) -> false
  | () ->
    let r_bytes, s_bytes = take_drop 32 signature in
    verify_limbs ~x:(limbs_of_be_string x) ~y:(limbs_of_be_string y)
      ~digest:(Sha2.Sha256.digest input)
      ~r:(limbs_of_be_string r_bytes)
      ~s:(limbs_of_be_string s_bytes)
