(* Generic little-endian 16-bit limb arithmetic, ported from tinysvid
   sig/limbs.ml. Big numbers: immutable int lists, base 2^16 limbs, LSB
   ...
   (RSA / ECDSA signature verification over public keys). *)
...
(* base ^ exp mod m by LSB-first square-and-multiply over the exponent
   bits. Arbitrary modulus: None when m is zero (mod_red would not
   terminate); Some canonical trimmed limbs otherwise (zero is []).
   NOT constant-time: verification handles public inputs only, and the
   exponent here is a public RSA verification exponent. *)
let mod_pow (m : int list) (base : int list) (e : int list) : int list option =
  match trim m with
  | [] -> None
  | _ :: _ ->
    let red = mod_red m in
    let result, _ =
      List.fold_left
        (fun (acc, sq) bit ->
          let acc' = if Int.equal bit 1 then red (mul_limbs acc sq) else acc in
          (acc', red (mul_limbs sq sq)))
        ([ 1 ], red base)
